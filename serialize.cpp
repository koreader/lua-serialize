...
unpack_table(lua_State *L, struct read_block *rb, int array_size) {
	if (array_size == MAX_COOKIE-1) {
		uint8_t type = 0;
		uint8_t *t = (uint8_t *)rb_read(rb, (char*)&type, sizeof(type));
		if (t==NULL) {
			invalid_stream(L,rb);
		}
		type = *t;
...
seristring(lua_State *L) {
	...
	lua_settop(L, 1);
	lserialize(L);
	auto buffer = (const char*)lua_touserdata(L, -2);
...
luaopen_serialize(lua_State *L) {
	luaL_Reg l[] = {
		...
		{ "readserifile", readserifile},
		{ "writeserifile",writeserifile},
